...
def reconstruct_with_index(column_data):
    reels = [{}, {}, {}]
    for stops, symbols in column_data.items():
        # 如果reels的长度小于symbols的长度，补充空字典，用于infinity reels
        more_reel = len(symbols) - len(reels)
        if more_reel > 0:
            for i in range(more_reel):
                reels.append({})
        ...
        for i in range(len(stops)):
            ...
            for j in range(len(reel)):
                if index+j not in reels[i].keys():
                    reels[i][index+j] = [reel[j], 1]
                else:
                    if reels[i][index+j][0] != reel[j]:
                        print("Error")
                    reels[i][index+j] = [reel[j], reels[i][index+j][1]+1]
