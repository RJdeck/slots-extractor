...
# raw_request["url"] & response_content["mimeType"] may need to be modified
def extract_to_log(save_file_name='slot_log.json', endswith='.har'):
    ...
    for file_name in files:
        ...
        for entry in entries:
            ...
            if raw_request["url"] != "https://demogamesfree.pragmaticplay.net/gs2c/ge/v4/gameService":
                continue
            try:
                raw_response = entry["response"]
                response_content = raw_response["content"]
                if response_content["mimeType"] != "text/plain" or not response_content.get("text"):
                    continue
...
def extrac_to_log_double(save_file_name='slot_log.json', endswith='.har'):
    files = os.listdir("input")
    ...
    for file_name in files:
        ...
        for entry in entries:
            if entry["_resourceType"] != "xhr":
                continue
            raw_request = entry["request"]
            if raw_request["url"] != "https://demogamesfree.pragmaticplay.net/gs2c/ge/v3/gameService":
                continue
            try:
                raw_response = entry["response"]
                response_content = raw_response["content"]
